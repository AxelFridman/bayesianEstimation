...
s = (as.character(2))
h = as.vector(df[s])
plot(grilla,h[[1]], col = rgb(red = 0, green = 0, blue = 1, alpha = 1/ncol(df)), ylim=c(0, 0.003) )

for (j in c(2:ncol(df))){
  s = (as.character(j))
  h = as.vector(df[s])
  points(grilla,h[[1]], col = rgb(red = 1-j/ncol(df), green = 1-j/ncol(df), blue = j/ncol(df), alpha = j/ncol(df)) )
}
